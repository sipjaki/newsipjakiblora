...
public function bepesertaskkshowberkas(Request $request, $id)
{
    ...
    // Ambil peserta berdasarkan ID dari tabel allskktenagakerjablora
    $datapeserta = allskktenagakerjablora::findOrFail($id);

    // Ambil juga data agendaskk terkait (jika diperlukan di view)
    $data = agendaskk::find($datapeserta->agendaskk_id);

    return view('backend.05_agenda.03_agendaskk.showpesertaberkas', [
        'title' => 'Berkas Peserta Sertifikasi',
        'data' => $data,
...
public function beakseslsppenerbitskk(Request $request)
{
    $perPage = $request->input('perPage', 15);
    $search = $request->input('search');
    $user = Auth::user();
 ...
 public function beagendapelatihanpesertadelete($id)
{
 ...
 public function besertifikatskk(Request $request, $id)
{
    // Data peserta berdasarkan id dari allskktenagakerjablora
    $datapesertaskk = allskktenagakerjablora::where('id', $id)->firstOrFail();

    // Ambil user_id dari query string
    $userId = $request->query('user_id');

    // (Optional) Ambil data user kalau kamu perlu
    $user = User::find($userId);

    return view('backend.05_agenda.04_pesertaskk.01_sertifikatskk.sertifikatskk', [
        'title' => 'Upload Sertifikat SKK',
        'data' => $datapesertaskk,
        'user' => $user, // atau tetap pakai Auth::user() kalau ini hanya untuk login
    ]);
}
