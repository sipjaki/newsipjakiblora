...
<section style="background: linear-gradient(to bottom, #a8f0c6, #ffffff); width: 100%; min-height: 100vh;">
        ...
        {{-- ======================================================= --}}
                    <div class="col-md-12">
                        <!--begin::Quick Example-->
                        <form action="{{ route('beuploadberkaspenyelenggaraan4upload', $data->id) }}" method="POST" enctype="multipart/form-data">
                            @csrf
...
<div class="col-md-6">
    <div class="mb-3">
        <label class="form-label" for="berkasdukung4">
            <i class="bi bi-file-earmark-check" style="margin-right: 10px; color: navy;"></i>
            Berkas Dukung Pengawasan Terhadap Manajemen Mutu Konstruksi
        </label>

        <!-- Tampilkan data sebelumnya -->
        <div style="margin-top: 10px;">
            @if(isset($data->berkasdukung4) && file_exists(public_path('storage/' . $data->berkasdukung4)))
                <!-- File ada di storage -->
                <iframe src="{{ asset('storage/' . $data->berkasdukung4) }}" frameborder="0" width="100%" height="200px" style="border:1px solid #ccc; margin-bottom:10px;"></iframe>

                <!-- Tombol download -->
                <a href="{{ asset('storage/' . $data->berkasdukung4) }}"
                   class="button-baru"
                   download>
                    <i class="bi bi-download"></i> Download Berkas
                </a>
            @elseif(isset($data->berkasdukung4))
                <!-- File ada tapi bukan di storage -->
                <iframe src="{{ asset($data->berkasdukung4) }}" frameborder="0" width="100%" height="750px" style="border:1px solid #ccc; margin-bottom:10px;"></iframe>

                <!-- Tombol download -->
                <a href="{{ asset($data->berkasdukung4) }}"
                   class="button-baru"
                   download>
                    <i class="bi bi-download"></i> Download Berkas
                </a>
        ...
        <!-- Input file baru -->
        <input type="file" id="berkasdukung4" name="berkasdukung4"
               class="form-control @error('berkasdukung4') is-invalid @enderror"
               accept=".pdf,.doc,.docx"
...
<!-- Script preview khusus berkasdukung4 -->
<script>
