...
<section style="background: linear-gradient(to bottom, #a8f0c6, #ffffff); width: 100%; min-height: 100vh;">
        ...
        {{-- ======================================================= --}}
                    <div class="col-md-12">
                        <!--begin::Quick Example-->
                        <form action="{{ route('beuploadberkaspenyelenggaraan5upload', $data->id) }}" method="POST" enctype="multipart/form-data">
                            @csrf
...
<div class="col-md-6">
    <div class="mb-3">
        <label class="form-label" for="berkasdukung5">
            <i class="bi bi-file-earmark-check" style="margin-right: 10px; color: navy;"></i>
            Berkas Dukung Pengawasan Terhadap Penggunaan Material Peralatan
        </label>

        <!-- Tampilkan data sebelumnya -->
        <div style="margin-top: 10px;">
            @if(isset($data->berkasdukung5) && file_exists(public_path('storage/' . $data->berkasdukung5)))
                <!-- File ada di storage -->
                <iframe src="{{ asset('storage/' . $data->berkasdukung5) }}" frameborder="0" width="100%" height="200px" style="border:1px solid #ccc; margin-bottom:10px;"></iframe>

                <!-- Tombol download -->
                <a href="{{ asset('storage/' . $data->berkasdukung5) }}"
                   class="button-baru"
                   download>
                    <i class="bi bi-download"></i> Download Berkas
                </a>
            @elseif(isset($data->berkasdukung5))
                <!-- File ada tapi bukan di storage -->
                <iframe src="{{ asset($data->berkasdukung5) }}" frameborder="0" width="100%" height="750px" style="border:1px solid #ccc; margin-bottom:10px;"></iframe>

                <!-- Tombol download -->
                <a href="{{ asset($data->berkasdukung5) }}"
                   class="button-baru"
                   download>
                    <i class="bi bi-download"></i> Download Berkas
                </a>
        ...
        <!-- Input file baru -->
        <input type="file" id="berkasdukung5" name="berkasdukung5"
               class="form-control @error('berkasdukung5') is-invalid @enderror"
               accept=".pdf,.doc,.docx"
...
<!-- Script preview khusus berkasdukung5 -->
<script>
