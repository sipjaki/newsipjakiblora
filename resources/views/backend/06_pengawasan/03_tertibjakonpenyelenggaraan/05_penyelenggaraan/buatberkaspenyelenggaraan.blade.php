...
<section style="background: linear-gradient(to bottom, #a8f0c6, #ffffff); width: 100%; min-height: 100vh;">
                            ...
                            <div style="display: flex; justify-content: space-between; margin-top: 10px; flex-wrap: wrap; margin-bottom:20px;">
                                ...
                                <div style="display: flex; gap: 10px; margin-right:25px; margin-left:25px;">
                                    ...
                                    <button class="button-berkas">
                                        ...
                                        Nama Proyek Konstruksi: {{ $datatertibjasakonstruksinamabadanusaha ?? '-' }}
                                    </button>
                                    ...
                                    <div class="modal-dialog modal-xl modal-dialog-centered">
                                        <div class="modal-content" style="font-size: 0.875rem;">
                                            ...
                                            <div class="modal-body">
                                                <h5 style="font-size: 1rem;">Surat Dukung Tertib Jakon Penyelenggaraan : Pengawasan Tertib Penyelenggaraan terhadap Kesesuaian Jenis, Sifat, Klasifikasi, dan Layanan Usaha dengan Kegiatan Usaha Jasa Konstruksi Secara Rutin</h5>
        ...
        {{-- ======================================================= --}}
                    <div class="col-md-12">
                        <!--begin::Quick Example-->
                        <form action="{{ route('betertibjakonpenyelenggaraan5createnew') }}" method="POST" enctype="multipart/form-data">
                            @csrf

                            <input type="hidden"  name="tertibjakonpenyelenggaraan_id" value="{{ $datatertibjasakonstruksi_id }}">
                                ...
                                <div class="modal-content" style="font-size: 0.875rem;">
                                    ...
                                    <br>
                                    <div class="modal-body">
                                        <h5 style="font-size: 15px;">Surat Dukung Tertib Jakon Penyelenggaraan : Pengawasan Tertib Penyelenggaraan Konstruksi Secara Rutin <br> Penyelenggaraan Konstruksi dengan Sumber Dana dari APBD </h5>
                                            <br>
                                        <table class="table table-bordered table-sm" style="font-size: 14px;">
                                            <tr>
    ...
    <input
        type="text"
        name="namaproyekkonstruksi"
        class="form-control @error('namaproyekkonstruksi') is-invalid @enderror"
        placeholder="Masukan Nama Proyek Konstruksi ..."
        value="{{ old('namaproyekkonstruksi', $datatertibjasakonstruksinamabadanusaha ?? 'Data Masih Kosong') }}"
        readonly>
...
<script>
    const input = document.getElementById('lingkuppengawasan');
    ...
    // Tampilkan format ribuan saat user mengetik
    input.addEventListener('input', function(e) {
        ...
        // Menjaga posisi cursor
        let newLength = input.value.length;
        input.selectionEnd = caretPosition + (newLength - originalLength);
    });

    // Saat submit form, hilangkan titik ribuan supaya yang dikirim ke DB hanya angka
...
<tr>
    ...
    <td>
        <textarea class="form-control @error('indikator') is-invalid @enderror"
                  name="indikator"
                  rows="1"
                  placeholder="Masukkan Nomor Kontrak Pekerjaan...">{{ old('indikator') }}</textarea>
        @error('indikator')
            <div class="invalid-feedback">{{ $message }}</div>
        @enderror
    </td>
...
<tr>
    <td>
        <strong style="font-size: 15px;">Penyedian Jasa CV/PT </strong>
    </td>
    <td>
        <textarea class="form-control @error('carapemerksaan1') is-invalid @enderror"
                  name="carapemerksaan1"
                  rows="3"
                  placeholder="Masukkan Nama Badan Usaha Jasa Konstruksi ...">{{ old('carapemerksaan1') }}</textarea>
...
<tr>
    ...
    <td>
        <textarea class="form-control @error('carapemerksaan2') is-invalid @enderror"
                  ...
                  rows="3"
                  placeholder="Masukkan OPD Contoh : Dinas Pekerjaan Umum Dan Penataan Ruang Kabupaten Blora ...">{{ old('carapemerksaan2') }}</textarea>
...
{{-- <tr>
    ...
    <textarea
        ...
        placeholder="Masukan Nama Proyek Konstruksi ..."
        rows="6"
        style="width: 100%; resize: vertical;" readonly>{{ old('namaproyekkonstruksi', $datatertibjasakonstruksi ?? 'Data Masih Kosong') }}</textarea>
    @error('namaproyekkonstruksi')
        <div class="invalid-feedback">{{ $message }}</div>
    @enderror
</td>
...
<table style="width: 100%; border-collapse: collapse; font-size: 13px; margin-bottom: 20px; border: 1px solid #000;">
    <thead>
        <tr>
            <th style="border: 1px solid #000; text-align: center; background-color: #f8f9fa; padding: 5px;">No</th>
            <th style="border: 1px solid #000; text-align: center; background-color: #f8f9fa; padding: 5px;">Lingkup Pengawasan</th>
            <th style="border: 1px solid #000; text-align: center; background-color: #f8f9fa; padding: 5px;">Indikator</th>
            <th style="border: 1px solid #000; text-align: center; background-color: #f8f9fa; padding: 5px;">Dokumen yang diperiksa</th>
            <th style="border: 1px solid #000; text-align: center; background-color: #f8f9fa; padding: 5px;">Cara Pemeriksaan</th>
            <th style="border: 1px solid #000; text-align: center; background-color: #f8f9fa; padding: 5px; width: 150px;">Kesimpulan Pemeriksaan</th>
            <th style="border: 1px solid #000; text-align: center; background-color: #f8f9fa; padding: 5px; width: 200px;">Catatan Pemeriksaan</th>
        </tr>
    </thead>
   <tbody>
    <tr>
        ...
        <td style="border: 1px solid #000; text-align: center;">
            <select class="form-control @error('kesimpulan1') is-invalid @enderror" name="kesimpulan1">
                <option value="">-- Pilih --</option>
                <option value="Sesuai" {{ old('kesimpulan1') == 'Sesuai' ? 'selected' : '' }}>Tersedia</option>
                <option value="Tidak Sesuai" {{ old('kesimpulan1') == 'Tidak Sesuai' ? 'selected' : '' }}>Tidak Tersedia</option>
            </select>
            @error('kesimpulan1')
                <div class="invalid-feedback">{{ $message }}</div>
            @enderror
        ...
        <td style="border: 1px solid #000;">
            <textarea
                ...
                name="kesimpulan2"
                rows="3"
                placeholder="Masukkan Catatan Pemeriksaan...">{{ old('kesimpulan2') }}</textarea>
            @error('kesimpulan2')
                <div class="invalid-feedback">{{ $message }}</div>
            @enderror
        </td>
    ...
    <tr>
        ...
        <td style="border: 1px solid #000; text-align: center;">
            <select class="form-control @error('catatan1') is-invalid @enderror" name="catatan1">
                <option value="">-- Pilih --</option>
                <option value="Sesuai" {{ old('catatan1') == 'Sesuai' ? 'selected' : '' }}>Sesuai</option>
                <option value="Tidak Sesuai" {{ old('catatan1') == 'Tidak Sesuai' ? 'selected' : '' }}>Tidak Sesuai</option>
            </select>
            @error('catatan1')
                <div class="invalid-feedback">{{ $message }}</div>
            @enderror
        ...
        <td style="border: 1px solid #000;">
            <textarea
                ...
                name="catatan2"
                rows="3"
                placeholder="Masukkan Catatan Pemeriksaan...">{{ old('catatan2') }}</textarea>
            @error('catatan2')
                <div class="invalid-feedback">{{ $message }}</div>
            @enderror
        </td>
