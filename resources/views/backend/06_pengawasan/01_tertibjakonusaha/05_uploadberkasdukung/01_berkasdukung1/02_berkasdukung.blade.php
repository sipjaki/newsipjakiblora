...
<section style="background: linear-gradient(to bottom, #a8f0c6, #ffffff); width: 100%; min-height: 100vh;">
        ...
        {{-- ======================================================= --}}
                    <div class="col-md-12">
                        <!--begin::Quick Example-->
                        <form action="{{ route('beuploadberkasusaha2new', $data->id) }}" method="POST" enctype="multipart/form-data">
                            @csrf
...
<div class="col-md-6">
    <div class="mb-3">
        ...
        <div style="margin-top: 10px;">
            @if(isset($data->cadangan2) && file_exists(public_path('storage/' . $data->cadangan2)))
                <!-- File ada di storage -->
                <iframe src="{{ asset('storage/' . $data->cadangan2) }}" frameborder="0" width="100%" height="200px" style="border:1px solid #ccc; margin-bottom:10px;"></iframe>

                <!-- Tombol download -->
                <a href="{{ asset('storage/' . $data->cadangan2) }}"
                   class="button-baru"
                   download>
                    <i class="bi bi-download"></i> Download Berkas
                </a>
            @elseif(isset($data->cadangan2))
                <!-- File ada tapi bukan di storage -->
                <iframe src="{{ asset($data->cadangan2) }}" frameborder="0" width="100%" height="200px" style="border:1px solid #ccc; margin-bottom:10px;"></iframe>

                <!-- Tombol download -->
                <a href="{{ asset($data->cadangan2) }}"
                   class="button-baru"
                   download>
                    <i class="bi bi-download"></i> Download Berkas
                </a>
        ...
        <!-- Input file baru -->
        <input type="file" id="cadangan2" name="cadangan2"
        ...
        <!-- Preview file baru -->
        <div id="preview-container-2" style="margin-top:10px;"></div>
    </div>
</div>

<!-- Script preview khusus cadangan2 -->
<script>
function previewFile2(event) {
    let file = event.target.files[0];
    let previewContainer = document.getElementById('preview-container-2');
    ...
    if (file) {
        ...
        if (file.type === "application/pdf") {
            previewContainer.innerHTML = `<iframe src="${fileURL}" frameborder="0" width="100%" height="200px" style="border:1px solid #ccc;"></iframe>`;
        } else {
