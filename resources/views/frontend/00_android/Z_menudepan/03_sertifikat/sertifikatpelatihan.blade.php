...
<div class="flex flex-col gap-1">
    @foreach ($data as $index => $data)
        ...
        <!-- Konten sertifikat yang tersembunyi awalnya -->
        <div id="sertifikat-{{ $data->id }}" style="display: none; border: 2px solid #ccc; padding: 20px; border-radius: 12px; margin-bottom: 40px; background: #fff;">
            <div class="cert-container">
                <div class="cert-header" style="text-align: center;">
                    <div class="cert-logos" style="margin-bottom: 10px;">
                ...
                <div class="cert-content" style="text-align: center;">
                    ...
                    <p style="margin: 10px 50px; text-align: justify; text-indent: 50px;">
                        Kegiatan <strong>{{ $data->agendapelatihan->namakegiatan }}</strong> yang diselenggarakan oleh
                        <strong>{{ $data->agendapelatihan->asosiasimasjaki->namaasosiasi ?? 'Nama Asosiasi Tidak Tersedia' }}</strong>
                        pada tanggal {{ \Carbon\Carbon::parse($data->agendapelatihan->waktupelaksanaan)->locale('id')->isoFormat('D MMMM YYYY') }},
                ...
                <div class="cert-content">
                    <div class="table-container">
                        <table class="custom-table">
                            ...
                            <tbody>
                                @php $totalJamPelajaran = 0; @endphp
                                @forelse ($data->agendapelatihan->jampelajaran as $key => $pelajaran)
                                    <tr class="table-row-hover">
                                        <td>{{ $key + 1 }}</td>
                                        <td>{{ $pelajaran->materi ?? 'Data Tidak Tersedia' }}</td>
                                        <td>{{ $pelajaran->narasumber ?? 'Data Tidak Tersedia' }}</td>
                                        <td style="text-align:center;">{{ $pelajaran->jampelajaran ?? 0 }} Jam</td>
                                    </tr>
                                    @php $totalJamPelajaran += (int) ($pelajaran->jampelajaran ?? 0); @endphp
                                @empty
                                    <tr>
                            ...
                            <tfoot>
                                <tr>
                                    <td colspan="3" style="text-align: right; font-weight: bold;">Total Jam Pelajaran :</td>
                                    <td style="text-align:center; font-weight: bold;">{{ $totalJamPelajaran }} Jam</td>
                                </tr>
                            </tfoot>
                        </table>
                    </div>
                ...
                <div class="ribbon-text" style="margin-top: 30px;">
                    <h4 style="text-transform: uppercase;">{{ strtoupper($data->namalengkap) }}</h4>
                    <p>Dinas Pekerjaan Umum Dan Penataan Ruang Kabupaten Blora<br>
                    Diterbitkan Pada : {{ \Carbon\Carbon::parse($data->agendapelatihan->waktupelaksanaan)->locale('id')->isoFormat('D MMMM YYYY') }}</p>
                </div>
